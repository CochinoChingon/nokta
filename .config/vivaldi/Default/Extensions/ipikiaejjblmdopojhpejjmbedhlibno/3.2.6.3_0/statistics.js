...
function updateHourlyrateValue() {
	...
	timeWorthTable.append('<tr><th>The average number of hours saved a month using Spreed*:</th><td>'+hoursPerMonth.toFixed(2)+'</td></tr>');

	//worth
	worth = hoursPerMonth*hourlyrate;
	timeWorthTable.append('<tr class="border-top"><th>Monthly amount saved using Spreed:</th><td><span class="big">$'+worth.toFixed(2)+' per month</span></td></tr>');
...
function initStatistics() {
	...
	if (averageWpm>250) {
		...
		statisticsTable.before('<div class="content" id="congrats-info"><h2 class="subtitle"><i>'+randomCongrats+'</i> Using Spreed, you read <b>'+readingGreatness+'x</b> faster than the average person!</h2><div id="share-stats"><button class="button is-primary">Share your reading achievement!</button></div></div><br>');
	...
	timeSavedString = secondsToString(timeSaved);
	statisticsTable.append('<tr><th>Time saved by using Spreed: </th><td>'+timeSavedString+'</td></tr>');
