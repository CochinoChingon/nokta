...
function backgroundRedirectToPaid(featureName) {
	chrome.runtime.sendMessage({action: "redirectToPaid", featureName:featureName }, function(response) {
...
settingsStore.isInitialized.then(function() {
	...
	// console.log('finished initializing settings store');
	...
	if (JSON.stringify(getUserFeatures()).length !== JSON.stringify(getDefaultUserFeatures()).length && getUserLicense() !== null) {
		...
		for (let i=0; i<contentIframes.length; i++) {
			...
			// each body has divs
			const curBodyDivs = $(contentIframes[i]).contents().find("body").find("div");
		...
		content = content.join(" "); // join with space. join with paragraph separators later?
		// console.log(content);
		...
		backgroundRedirectToPaid('kindle_cloud_extract');
	}
